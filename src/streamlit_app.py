...
# ── plotly global font override (hitam untuk readability) ─────
PLOTLY_FONT = dict(family="Inter, Helvetica, sans-serif", size=11, color="#1A1A1A")
...
def pb(height=320, **kw):
    """Base plotly layout — hitam untuk semua teks agar kontras di background putih."""
    b = dict(
        height=height,
...
def palette_html(row, n=5):
    """Palette bar dengan tooltip nama warna + persentase (bukan kode hex)."""
    parts, total = [], 0.0
    for i in range(1,n+1):
        ...
        # cari nama warna terdekat dari WARNA_HEX
        nama = _nama_warna(hx)
        parts.append((hx, pct, nama))
        total += pct
    if not parts: return ""
...
def _nama_warna(hex_str):
    """Kembalikan nama warna Indonesia terdekat dari WARNA_HEX berdasarkan jarak RGB."""
    try:
        h = hex_str.lstrip("#")
    ...
    for nama, hx in WARNA_HEX.items():
        try:
            ...
            if d < best_d: best, best_d = nama, d
        except: pass
    return best
...
def heatmap_warna_genre(d, top_n=12):
    """Peta panas Warna × Genre: proporsi buku per genre yang memiliki warna dominan tsb."""
    ...
    # format sebagai persentase integer untuk label
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        showscale=True,
        colorbar=dict(tickfont=dict(color="#1A1A1A"), title=dict(text="Proporsi", font=dict(color="#1A1A1A"))),
    ))
    fig.update_layout(**pb(max(340, top_n*28), margin=dict(l=120,r=20,t=32,b=60)),
        xaxis=dict(tickfont=dict(color="#1A1A1A"), side="bottom"),
        yaxis=dict(tickfont=dict(color="#1A1A1A"), autorange="reversed"),
...
def heatmap_tf_genre(d, top_n=12):
    ...
    mat = pd.DataFrame(0.0, index=genres, columns=[TYPEFACE_ID[k] for k in tf_keys])
    ...
    fig = go.Figure(data=go.Heatmap(
        z=mat.values,
        x=[TYPEFACE_ID[k] for k in tf_keys],
        y=genres,
        colorscale="Purples",
        text=text_mat.values,
        texttemplate="%{text}",
    ...
    fig.update_layout(**pb(max(340, top_n*28), margin=dict(l=120,r=20,t=32,b=80)),
        xaxis=dict(tickfont=dict(color="#1A1A1A", size=9), tickangle=-30, side="bottom"),
        yaxis=dict(tickfont=dict(color="#1A1A1A"), autorange="reversed"),
        xaxis_title="", yaxis_title="")
    return fig

def heatmap_gaya_genre(d, top_n=12):
    """Peta panas Gaya Ilustrasi × Genre."""
    genres = _top_genres(d, top_n)
    ...
    fig = go.Figure(data=go.Heatmap(
        z=mat.values,
        x=[GAYA_ID[k] for k in gaya_keys],
        y=genres,
        colorscale="Greens",
        text=text_mat.values,
        texttemplate="%{text}",
...
if HAL == "Beranda":
    ...
    with ca:
        ...
        fig.update_layout(**pb(260), showlegend=True,
                          legend=dict(orientation="h",y=-.1,font=dict(color="#1A1A1A")))
    ...
    with cc:
        ...
        fig3.update_layout(**pb(260), showlegend=False, xaxis_title="", yaxis_title="",
...
# ── WARNA ─────────────────────────────────────────────────────
    ...
    with cb2:
        ...
        fig2.update_layout(**pb(310), xaxis_title="", yaxis_title="", showlegend=True,
                           legend=dict(orientation="h",y=-.2,font=dict(size=9,color="#1A1A1A")))
        st.plotly_chart(fig2, use_container_width=True)
    ...
    fig_sc.update_layout(**pb(300), showlegend=True,
                         legend=dict(orientation="h",y=-.18,font=dict(size=10,color="#1A1A1A")),
    ...
    st.markdown("<small>Proporsi buku per genre yang memiliki warna dominan tersebut. Menunjukkan asosiasi visual-tematik antara palet warna dan genre literatur.</small>", unsafe_allow_html=True)
    hn_w = st.slider("Jumlah genre yang ditampilkan", 6, 20, 12, 2, key="hn_warna")
...
# ── TIPOGRAFI ─────────────────────────────────────────────────
    ...
    with cb3:
        ...
        fig2.update_layout(**pb(300), xaxis_title="", yaxis_title="", showlegend=True,
                           legend=dict(orientation="h",y=-.22,font=dict(size=9,color="#1A1A1A")))
        st.plotly_chart(fig2, use_container_width=True)
    ...
    if prob_cols_tf:
        ...
        fp.update_layout(**pb(240), showlegend=False, xaxis_title="Rata-rata Softmax",
                         yaxis_title="",
    ...
    hn_tf = st.slider("Jumlah genre yang ditampilkan", 6, 20, 12, 2, key="hn_tf")
...
# ── ILUSTRASI ─────────────────────────────────────────────────
    ...
    hn_gi = st.slider("Jumlah genre yang ditampilkan", 6, 20, 12, 2, key="hn_gi")
    ...
    with man_a:
        ...
        fig_man.update_layout(**pb(240), barmode="group", showlegend=True,
                              legend=dict(orientation="h",y=-.15,font=dict(color="#1A1A1A")),
                              xaxis_title="", yaxis_title="")
...
# ── GENRE ─────────────────────────────────────────────────────
    ...
    # Semua genre (jenis + tematik) untuk peta panas tumpang tindih
    ...
    # ── Peta Panas Tumpang Tindih — SEMUA genre (jenis + tematik) ──
    st.markdown("<hr class='thin'>", unsafe_allow_html=True)
    st.markdown("**Peta Panas Tumpang Tindih Genre**")
    st.markdown("<small>Frekuensi kemunculan bersama antar genre (co-occurrence). Mencakup jenis karya dan genre tematik sekaligus.</small>", unsafe_allow_html=True)
    ...
    # isi diagonal dengan jumlah buku per genre agar lebih informatif
    for g in top_co:
        co.loc[g,g] = gc_all[g]
    text_co = co.astype(str)
    fig_co = go.Figure(data=go.Heatmap(
        ...
        colorbar=dict(tickfont=dict(color="#1A1A1A"),
                      title=dict(text="Jumlah", font=dict(color="#1A1A1A"))),
    ))
    fig_co.update_layout(
        **pb(max(420, n_co*28), margin=dict(l=120,r=20,t=32,b=120)),
        xaxis=dict(tickfont=dict(color="#1A1A1A", size=9), tickangle=-40, side="bottom"),
        yaxis=dict(tickfont=dict(color="#1A1A1A", size=9), autorange="reversed"),
        xaxis_title="", yaxis_title=""
    )
...
# ── ILLUSTRATOR ───────────────────────────────────────────────
    ...
    fig_wc.update_layout(**pb(280), barmode="group", showlegend=True, xaxis_title="",
    ...
    with il2a:
        ...
        fig_gw.update_layout(**pb(240), showlegend=True,
                             legend=dict(orientation="h",y=-.1,font=dict(size=10,color="#1A1A1A")))
