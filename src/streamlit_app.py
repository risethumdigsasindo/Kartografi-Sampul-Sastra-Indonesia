...
def compute_brightness_saturation_by_genre(d, top_n=18):
    """
    Hitung rata-rata kecerahan (brightness_mean) dan saturasi (saturation_mean)
    per genre, termasuk info klaster.

    Returns: DataFrame dengan kolom genre, brightness, saturation, n_buku, klaster_id, klaster_color
    """
    genres = _top_genres_filtered(d, top_n)
    genre_lists = expand_genres(d["GENRES"], normalize=True)
    rows = []
    for g in genres:
        ...
        if sub.empty:
            continue
        br = pd.to_numeric(sub["brightness_mean"], errors="coerce").dropna()
        sa = pd.to_numeric(sub["saturation_mean"], errors="coerce").dropna()
        kl = GENRE_KLASTER_MAP.get(g)
        rows.append({
            "genre":          g,
            "brightness":     br.mean() if len(br) > 0 else np.nan,
            "saturation":     sa.mean() if len(sa) > 0 else np.nan,
            "brightness_std": br.std()  if len(br) > 1 else 0,
            "saturation_std": sa.std()  if len(sa) > 1 else 0,
            "n_buku":         len(sub),
            "klaster_id":    kl["id"]    if kl else "–",
            "klaster_color": kl["color"] if kl else "#888",
            "klaster_bg":    kl["bg"]    if kl else "#f0f0f0",
        })
    return pd.DataFrame(rows).dropna(subset=["brightness","saturation"])


def compute_brightness_saturation_by_klaster(d):
    """
    Hitung rata-rata kecerahan dan saturasi per klaster genre.
    Setiap buku dihitung satu kali ke klaster yang paling banyak genre-nya.
    """
    genre_lists = expand_genres(d["GENRES"], normalize=True)
    klaster_data = {kl["id"]: {"br": [], "sa": [], "label": kl["label"],
                                "color": kl["color"], "bg": kl["bg"]}
                    for kl in KLASTER_COOC}
    for idx, row in d.iterrows():
        gl = genre_lists[d.index.get_loc(idx)] if idx in d.index else []
        # tentukan klaster dominan berdasarkan jumlah genre yang match
        best_kl, best_cnt = None, 0
        for kl in KLASTER_COOC:
            cnt = sum(1 for g in gl if g in kl["genres"])
            if cnt > best_cnt:
                best_cnt, best_kl = cnt, kl["id"]
        if best_kl is None:
            continue
        try:
            br = float(row.get("brightness_mean", np.nan) or np.nan)
            sa = float(row.get("saturation_mean", np.nan) or np.nan)
        except Exception:
            continue
        if not np.isnan(br):
            klaster_data[best_kl]["br"].append(br)
        if not np.isnan(sa):
            klaster_data[best_kl]["sa"].append(sa)

    rows = []
    for kid, kd in klaster_data.items():
        br_list = kd["br"]; sa_list = kd["sa"]
        rows.append({
            "klaster_id":    kid,
            "label":         kd["label"],
            "brightness":    np.mean(br_list) if br_list else np.nan,
            "saturation":    np.mean(sa_list) if sa_list else np.nan,
            "brightness_std": np.std(br_list) if len(br_list) > 1 else 0,
            "saturation_std": np.std(sa_list) if len(sa_list) > 1 else 0,
            "n_buku":        len(br_list),
            "color":         kd["color"],
            "bg":            kd["bg"],
        })
...
def render_brightness_saturation_section(d, top_n=18):
    """
    Render lengkap: heatmap kecerahan+saturasi per genre, chart per klaster,
    dan scatter plot brightness vs saturation berdasarkan klaster.
    """
    st.markdown("**Kecerahan & Saturasi per Genre**")
    st.markdown(
        "<small>Kecerahan (V) dan saturasi (S) dihitung dari rata-rata piksel sampul "
        "dalam ruang warna HSV. Nilai 0–1 dinormalisasi dari skala OpenCV (0–255). "
        "Semakin mendekati 1 = semakin cerah / jenuh warnanya.</small>",
        unsafe_allow_html=True
    )

    df_genre = compute_brightness_saturation_by_genre(d, top_n)
    df_kl    = compute_brightness_saturation_by_klaster(d)

    if df_genre.empty:
        st.caption("Data kecerahan/saturasi tidak tersedia.")
        return

    # ── Tab: Genre vs Klaster ──────────────────────────────────────────────
    tab1, tab2, tab3 = st.tabs(["📊 Per Genre", "🗂 Per Klaster", "🔵 Scatter Plot"])

    # ─────────────── TAB 1: Per Genre ────────────────────────────────────
    with tab1:
        sort_by = st.radio(
            "Urutkan berdasarkan", ["Kecerahan (tinggi→rendah)", "Saturasi (tinggi→rendah)", "Klaster"],
            horizontal=True, key="bs_sort"
        )
        if sort_by == "Kecerahan (tinggi→rendah)":
            df_plot = df_genre.sort_values("brightness", ascending=True)
        elif sort_by == "Saturasi (tinggi→rendah)":
            df_plot = df_genre.sort_values("saturation", ascending=True)
        else:
            order = {"K1": 0, "K2": 1, "K3": 2, "–": 3}
            df_plot = df_genre.copy()
            df_plot["_ord"] = df_plot["klaster_id"].map(order).fillna(9)
            df_plot = df_plot.sort_values(["_ord", "brightness"], ascending=[True, True])

        # Label genre + klaster
        df_plot["label_y"] = df_plot.apply(
            lambda r: f"{r['genre']}  [{r['klaster_id']}]" if r["klaster_id"] != "–"
                      else r["genre"], axis=1
        )

        col_br, col_sa = st.columns(2)

        with col_br:
            st.markdown("**Rata-rata Kecerahan (V)**")
            fig_br = go.Figure()
            fig_br.add_trace(go.Bar(
                x=df_plot["brightness"],
                y=df_plot["label_y"],
                orientation="h",
                marker_color=df_plot["klaster_color"].tolist(),
                marker_line_width=0,
                error_x=dict(type="data", array=df_plot["brightness_std"].tolist(),
                             visible=True, color="rgba(0,0,0,.25)", thickness=1.2),
                hovertemplate=(
                    "<b>%{y}</b><br>"
                    "Kecerahan: %{x:.3f}<br>"
                    "<extra></extra>"
            ...
            fig_br.add_vline(
                x=float(pd.to_numeric(d["brightness_mean"], errors="coerce").mean()),
                line_dash="dash", line_color="rgba(128,128,128,.5)",
                annotation_text="rata-rata korpus",
                annotation_font_size=9,
                annotation_position="top right"
            )
            fig_br.update_layout(
                **pb(max(320, len(df_plot) * 28),
                     margin=dict(l=170, r=20, t=30, b=30)),
                xaxis=dict(range=[0, 1.05], title="Kecerahan (0–1)"),
                yaxis=dict(title=""),
                showlegend=False,
            )
        ...
        with col_sa:
            st.markdown("**Rata-rata Saturasi (S)**")
            fig_sa = go.Figure()
            fig_sa.add_trace(go.Bar(
                x=df_plot["saturation"],
                y=df_plot["label_y"],
                orientation="h",
                marker_color=df_plot["klaster_color"].tolist(),
                marker_line_width=0,
                error_x=dict(type="data", array=df_plot["saturation_std"].tolist(),
                             visible=True, color="rgba(0,0,0,.25)", thickness=1.2),
                hovertemplate=(
                    "<b>%{y}</b><br>"
                    "Saturasi: %{x:.3f}<br>"
            ...
            fig_sa.add_vline(
                x=float(pd.to_numeric(d["saturation_mean"], errors="coerce").mean()),
                line_dash="dash", line_color="rgba(128,128,128,.5)",
                annotation_text="rata-rata korpus",
                annotation_font_size=9,
                annotation_position="top right"
            ...
            fig_sa.update_layout(
                **pb(max(320, len(df_plot) * 28),
                     margin=dict(l=170, r=20, t=30, b=30)),
                xaxis=dict(range=[0, 1.05], title="Saturasi (0–1)"),
                yaxis=dict(title=""),
                showlegend=False,
            )
            st.plotly_chart(fig_sa, use_container_width=True)

        # Legenda warna klaster
        leg_cols = st.columns(3)
        for lc, kl in zip(leg_cols, KLASTER_COOC):
            lc.markdown(
        ...
        # Heatmap gabungan brightness + saturation
        st.markdown("<br>**Heatmap Kecerahan & Saturasi × Genre**", unsafe_allow_html=True)
        st.markdown(
            "<small>Nilai dinormalisasi ke [0–1]. "
            "Warna sel = intensitas nilai (semakin gelap = semakin tinggi).</small>",
            unsafe_allow_html=True
        )
        ...
        fig_hm = go.Figure(data=go.Heatmap(
            ...
            texttemplate="%{text}",
            textfont=dict(size=9, color="#1A1A1A"),
            hovertemplate="Genre: %{x}<br>Dimensi: %{y}<br>Nilai: %{text}<extra></extra>",
            showscale=True,
            zmin=0, zmax=1,
        ))
        fig_hm.update_layout(
            **pb(180, margin=dict(l=120, r=20, t=30, b=80)),
    ...
    with tab2:
        if df_kl.empty:
            ...
            st.markdown(
                "<small>Setiap buku dipetakan ke klaster berdasarkan genre dominannya. "
                "Error bar = standar deviasi.</small>",
                unsafe_allow_html=True
            )
            kl_cols2 = st.columns(len(df_kl))
            for col_k, (_, row_k) in zip(kl_cols2, df_kl.iterrows()):
                with col_k:
                    st.markdown(
                        f'<div style="border:1px solid {row_k["color"]}22;border-radius:10px;'
                        f'background:{row_k["bg"]};padding:.7rem .9rem;text-align:center;">'
                        f'<div style="font-size:11px;font-weight:700;color:{row_k["color"]}">'
                        f'[{row_k["klaster_id"]}]</div>'
                        f'<div style="font-size:.65rem;opacity:.65;margin:.2rem 0 .5rem;line-height:1.3">'
                        f'{row_k["label"].split("—")[1].strip() if "—" in row_k["label"] else row_k["label"]}</div>'
                        f'<div style="font-size:1.5rem;font-weight:700;color:{row_k["color"]}">'
                        f'{row_k["brightness"]:.3f}</div>'
                        f'<div style="font-size:.6rem;opacity:.5;margin-bottom:.4rem">kecerahan ± {row_k["brightness_std"]:.3f}</div>'
                        f'<div style="font-size:1.1rem;font-weight:600;color:{row_k["color"]}">'
                        f'{row_k["saturation"]:.3f}</div>'
                        f'<div style="font-size:.6rem;opacity:.5">saturasi ± {row_k["saturation_std"]:.3f}</div>'
                        f'<div style="font-size:.6rem;opacity:.4;margin-top:.3rem">{row_k["n_buku"]:,} buku</div>'
                        f'</div>',
                        unsafe_allow_html=True
                    )
            ...
            # Bar chart perbandingan klaster
            fig_kl = go.Figure()
            fig_kl.add_trace(go.Bar(
                name="Kecerahan (V)",
                x=df_kl["klaster_id"],
                y=df_kl["brightness"],
                error_y=dict(type="data", array=df_kl["brightness_std"].tolist(),
                             visible=True, color="rgba(0,0,0,.3)"),
                marker_color=[row["color"] for _, row in df_kl.iterrows()],
                marker_line_width=0,
                opacity=0.85,
            ))
            fig_kl.add_trace(go.Bar(
                ...
                x=df_kl["klaster_id"],
                y=df_kl["saturation"],
                error_y=dict(type="data", array=df_kl["saturation_std"].tolist(),
                             visible=True, color="rgba(0,0,0,.3)"),
                marker_color=[row["color"] for _, row in df_kl.iterrows()],
                marker_line_width=0,
                opacity=0.40,
            ...
            fig_kl.update_layout(
                **pb(300, margin=dict(l=40, r=20, t=30, b=50)),
                barmode="group",
                xaxis_title="Klaster Genre",
                yaxis=dict(title="Nilai (0–1)", range=[0, 1.1]),
                legend=dict(orientation="h", y=-0.2, font=dict(size=10)),
            )
            st.plotly_chart(fig_kl, use_container_width=True)

            # Radar chart
            st.markdown("**Radar: Kecerahan vs Saturasi per Klaster**")
            fig_radar = go.Figure()
            for _, row_k in df_kl.iterrows():
                fig_radar.add_trace(go.Scatterpolar(
                    r=[row_k["brightness"], row_k["saturation"],
                       row_k["brightness"]],  # tutup loop
                    theta=["Kecerahan", "Saturasi", "Kecerahan"],
                    fill="toself",
                    name=f'[{row_k["klaster_id"]}] {row_k["label"].split("—")[1].strip() if "—" in row_k["label"] else ""}',
                    line_color=row_k["color"],
                    fillcolor=row_k["color"] + "33",
                ))
            fig_radar.update_layout(
                **pb(340, margin=dict(l=60, r=60, t=50, b=60)),
    ...
    with tab3:
        st.markdown(
            "<small>Setiap titik = satu buku. Posisi X = kecerahan, Y = saturasi. "
            "Warna = klaster genre dominan buku tersebut.</small>",
            unsafe_allow_html=True
        )

        # Tentukan klaster dominan per buku untuk scatter
        genre_lists_sc = expand_genres(d["GENRES"], normalize=True)
        d_sc = d.copy()
        kl_colors_map = {kl["id"]: kl["color"] for kl in KLASTER_COOC}
        kl_label_map  = {kl["id"]: kl["label"].split("—")[1].strip() if "—" in kl["label"] else kl["label"]
                         for kl in KLASTER_COOC}

        kl_assign = []
        for idx in d_sc.index:
            loc = d_sc.index.get_loc(idx)
            gl  = genre_lists_sc[loc] if loc < len(genre_lists_sc) else []
            best_kl, best_cnt = "–", 0
            for kl in KLASTER_COOC:
                cnt = sum(1 for g in gl if g in kl["genres"])
                if cnt > best_cnt:
                    best_cnt, best_kl = cnt, kl["id"]
            kl_assign.append(best_kl)
        d_sc["_klaster"] = kl_assign

        d_sc_plot = d_sc.dropna(subset=["brightness_mean", "saturation_mean"]).copy()
        d_sc_plot["brightness_mean"] = pd.to_numeric(d_sc_plot["brightness_mean"], errors="coerce")
        d_sc_plot["saturation_mean"] = pd.to_numeric(d_sc_plot["saturation_mean"], errors="coerce")
        d_sc_plot = d_sc_plot.dropna(subset=["brightness_mean", "saturation_mean"])

        fig_sc2 = go.Figure()
        for kl in KLASTER_COOC:
            sub_sc = d_sc_plot[d_sc_plot["_klaster"] == kl["id"]]
            if sub_sc.empty:
                continue
            fig_sc2.add_trace(go.Scatter(
                x=sub_sc["brightness_mean"],
                y=sub_sc["saturation_mean"],
                mode="markers",
                name=f'[{kl["id"]}] {kl_label_map[kl["id"]]}',
                marker=dict(
                    color=kl["color"],
                    size=4,
                    opacity=0.35,
                ...
                customdata=sub_sc[["TITLE", "AUTHOR", "YEAR"]].values,
                hovertemplate=(
                    "<b>%{customdata[0]}</b><br>"
                    "%{customdata[1]} · %{customdata[2]}<br>"
                    f"Klaster: [{kl['id']}]<br>"
                    "Kecerahan: %{x:.3f}<br>"
                    "Saturasi: %{y:.3f}<extra></extra>"
                )
            ))
        # Tambah titik rata-rata per klaster
        for _, row_k in df_kl.iterrows():
            fig_sc2.add_trace(go.Scatter(
                x=[row_k["brightness"]],
                y=[row_k["saturation"]],
                mode="markers+text",
                name=f'Rata-rata [{row_k["klaster_id"]}]',
                marker=dict(
                    color=row_k["color"],
                    size=14,
                    symbol="diamond",
                ...
                text=[f'[{row_k["klaster_id"]}]'],
                textposition="top center",
                textfont=dict(size=9, color=row_k["color"]),
                showlegend=False,
            ))
        fig_sc2.update_layout(
            **pb(420, margin=dict(l=50, r=20, t=30, b=60)),
            xaxis=dict(title="Kecerahan (V)", range=[0, 1.05]),
            yaxis=dict(title="Saturasi (S)", range=[0, 1.05]),
            showlegend=True,
        ...
        for kl in KLASTER_COOC:
            ...
            if sub_sc.empty:
                continue
            stats_rows.append({
                "Klaster": f'[{kl["id"]}] {kl_label_map[kl["id"]]}',
                "N Buku": len(sub_sc),
                "Kecerahan (mean)": f'{sub_sc["brightness_mean"].mean():.3f}',
                "Kecerahan (std)":  f'{sub_sc["brightness_mean"].std():.3f}',
                "Saturasi (mean)":  f'{sub_sc["saturation_mean"].mean():.3f}',
                "Saturasi (std)":   f'{sub_sc["saturation_mean"].std():.3f}',
            })
        if stats_rows:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── KECERAHAN & SATURASI PER GENRE & KLASTER ──────────────────────────
    st.markdown("<hr class='thin'>", unsafe_allow_html=True)
    hn_bs = st.slider("Jumlah genre (kecerahan & saturasi)", 6, 25, 18, 1, key="hn_bs")
    render_brightness_saturation_section(DF, top_n=hn_bs)
